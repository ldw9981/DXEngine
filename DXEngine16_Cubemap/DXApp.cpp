...
bool DXApp::InitScene()
{
	...
	hr = D3DCompileFromFile(L"CubeVS.fx", NULL, NULL,
	...
	// 픽셀 셰이더 컴파일.
	hr = D3DCompileFromFile(L"CubePS.fx", NULL, NULL,
...
bool DXApp::InitTexture()
{
	// 텍스처 파일 로드.
	HRESULT hr;
	hr = CreateDDSTextureFromFile(pDevice, L"SaintPetersBasilica.dds", nullptr, &pTexture);
	if (FAILED(hr))
	{
